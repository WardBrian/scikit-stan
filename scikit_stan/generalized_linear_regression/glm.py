...
class GLM(CoreEstimator):
    ...
    def _predict_scipy(
        ...
        links = {
            ...
            "sqrt": np.square,
            "inverse-square": lambda x: 1 / (x**2),
            "logit": scipy.special.expit,
            "probit": stats.norm.cdf,
            "cauchit": lambda x: np.atan(x) / np.pi + 0.5,
            "cloglog": lambda x: 1 - np.exp(-np.exp(x)),
        }

        families = {
            "gaussian": lambda a: stats.norm.rvs(
                a, self.sigma_, random_state=self.seed_
            ),
            "gamma": lambda a: stats.gamma.rvs(a, self.sigma_, random_state=self.seed_),
            "inverse-gaussian": lambda a: stats.wald.rvs(
                a, self.sigma_, random_state=self.seed_
            ),
            "poisson": lambda a: stats.poisson.rvs(a, random_state=self.seed_),
            "binomial": lambda a: stats.binom.rvs(a, random_state=self.seed_),
            # "negative-binomial": ,
            "bernoulli": lambda a: stats.bernoulli.rvs(a, random_state=self.seed_),
        }

        return families[self.family](
            self._alpha + links[self.link](np.dot(self.beta_, X))
        )
    ...
    def predict_distribution(
        ...
        if self.algorithm != "sample":
            return self._predict_scipy(X_clean)
